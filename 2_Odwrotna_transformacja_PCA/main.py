...
# Centrowanie danych 
# (zapisanie: `X_centered = X - np.mean(X, axis=0)` w inny sposób, w celu zapisania średniej)
...
# Wariancje składowych PCA
variances = pca.explained_variance_

# Indeks składowej o najmniejszej wariancji
idx_min = np.argmin(variances)

# Usuwanie najsłabszej składowej
Y_reduced = Y.copy()
Y_reduced[:, idx_min] = 0

# Transformacja odwrotna: rekonstrukcja X
X_reconstructed = np.dot(Y_reduced, pca.components_) + mean

# Przeliczenie różnic
diff = X - X_reconstructed

print("\nRóżnice pomiędzy X a X_reconstructed:")
print(diff)


# Wykres porównujący dane oryginalne i odtworzone
cechy = ['x1', 'x2', 'x3']

fig, axes = plt.subplots(1, 3, figsize=(15, 4))

for i in range(3):
    axes[i].plot(X[:, i], marker='o', label='Oryginalne')
    axes[i].plot(X_reconstructed[:, i], marker='x', label='Odtworzone')
    axes[i].set_title(f'Porównanie cechy {cechy[i]}')
    axes[i].set_xlabel('Instancja')
    axes[i].set_ylabel(cechy[i])
    axes[i].legend()
    axes[i].grid(True)
...
# Różnice dla x1 i x2 są niewielkie - dobra rekonstrukcja dla tych cech.
# Największe błędy są dla x3 - przez usunięcie składowej PCA o najmniejszej wariancji.